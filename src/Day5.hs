...
jumpIfTrue :: State -> [Int] -> State   
jumpIfTrue state modes = do
    let firstParam =  getParameterValue (program state) (counter state) modes 0
    if firstParam /= 0 then do
...
jumpIfFalse :: State -> [Int] -> State   
jumpIfFalse state modes = do
    let firstParam =  getParameterValue (program state) (counter state) modes 0
    if firstParam == 0 then do
        let sndParam = getParameterValue (program state) (counter state) modes 1
        state {counter = sndParam}
...
lessThan :: State -> [Int] -> State   
lessThan state modes = do
    let firstParam =  getParameterValue (program state) (counter state) modes 0
    let sndParam = getParameterValue (program state) (counter state) modes 1
    let thirdParam = (program state)!!((counter state)+ 3)
    if firstParam < sndParam then
...
equals :: State -> [Int] -> State   
equals state modes = do
    let firstParam =  getParameterValue (program state) (counter state) modes 0
    let sndParam = getParameterValue (program state) (counter state) modes 1
    let thirdParam = (program state)!!((counter state)+ 3)
    if firstParam == sndParam then
...
stepThroughProgram state = do
    ...
    case op of 
        ...
        4 -> do
            ...
            stepThroughProgram (newState)
        5 -> do 
            let newState = jumpIfTrue state modes
            stepThroughProgram (newState)
        6 -> do
            let newState = jumpIfFalse state modes
            stepThroughProgram (newState)
        7 -> do 
            let newState = lessThan state modes
            stepThroughProgram (newState)
        8 -> do
            let newState = equals state modes
            stepThroughProgram (newState)
        _ -> do
